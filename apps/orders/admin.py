...
class OrderAdmin(admin.ModelAdmin):
    list_display = ["order_number", "user", "items_summary", "status_badge", "payment_status", "total", "tracking_number", "courier_name", "created_at", "invoice_link"]
    ...
    def items_summary(self, obj):
        ...
        if not items:
            return "—"
        text = ", ".join(f"{i.product_name} ×{i.quantity}" for i in items)
    ...
    def invoice_link(self, obj):
        return format_html('<a href="/orders/{}/invoice/" target="_blank" style="color:#B8945A;font-weight:600">⬇ Invoice</a>', obj.pk)
    ...
    actions = ["ship_with_tracking", "packing_slips", "mark_confirmed", "mark_processing",
               "mark_shipped", "mark_out_for_delivery", "mark_delivered", "mark_cancelled",
               "export_orders_csv"]
    ...
    def ship_with_tracking(self, request, queryset):
        if "apply" in request.POST:
            ...
            shipped = 0
            for o in queryset:
                tracking = request.POST.get(f"tracking_{o.pk}", "").strip()
                o.tracking_number = tracking
                o.courier_name = courier
                if tracking and courier:
                    o.tracking_url = f"https://www.google.com/search?q={courier}+{tracking}"
                if est:
                    o.estimated_delivery = est
                o.status = "shipped"
                o.save()  # fires shipped email (includes tracking)
                OrderStatusHistory.objects.create(order=o, status="shipped",
                    note=f"Shipped via {courier} {tracking}".strip(), created_by=request.user)
                shipped += 1
            self.message_user(request, f"{shipped} order(s) marked shipped with tracking. Customers emailed.", messages.SUCCESS)
            return None
        return render(request, "admin/ship_with_tracking.html", {
            "orders": queryset, "action_checkbox_name": ACTION_CHECKBOX_NAME,
    ...
    def _bulk_status(self, request, queryset, new_status, verb):
        ...
        for o in queryset:
            if o.status != new_status:
                o.status = new_status
                o.save()  # triggers per-order signals
                OrderStatusHistory.objects.create(order=o, status=new_status, created_by=request.user)
                n += 1
        self.message_user(request, f"{n} order(s) marked {verb}. Customers notified where applicable.")

    @admin.action(description="→ Mark CONFIRMED")
    def mark_confirmed(self, request, qs): self._bulk_status(request, qs, "confirmed", "confirmed")
    @admin.action(description="→ Mark PROCESSING")
    def mark_processing(self, request, qs): self._bulk_status(request, qs, "processing", "processing")
    @admin.action(description="📦 Mark SHIPPED (add tracking in the list, then run)")
    def mark_shipped(self, request, qs): self._bulk_status(request, qs, "shipped", "shipped")
    @admin.action(description="🚚 Mark OUT FOR DELIVERY")
    def mark_out_for_delivery(self, request, qs): self._bulk_status(request, qs, "out_for_delivery", "out for delivery")
    @admin.action(description="✓ Mark DELIVERED")
    def mark_delivered(self, request, qs): self._bulk_status(request, qs, "delivered", "delivered")
    @admin.action(description="✗ Mark CANCELLED (restocks)")
    def mark_cancelled(self, request, qs): self._bulk_status(request, qs, "cancelled", "cancelled")

    @admin.action(description="⬇ Export selected orders to CSV")
    def export_orders_csv(self, request, queryset):
        resp = HttpResponse(content_type="text/csv")
        resp["Content-Disposition"] = 'attachment; filename="aura-orders.csv"'
        w = csv.writer(resp)
        w.writerow(["Order", "Date", "Customer", "Email", "Phone", "Status", "Payment",
                    "Method", "Subtotal", "Discount", "Tax", "Shipping", "Total", "Items", "City", "Pincode"])
        for o in queryset.select_related("user").prefetch_related("items"):
            items = " | ".join(f"{i.product_name} x{i.quantity}" for i in o.items.all())
            w.writerow([o.order_number, o.created_at.strftime("%Y-%m-%d %H:%M"), o.shipping_full_name,
                        o.user.email if o.user else "", o.shipping_phone, o.get_status_display(),
                        o.get_payment_status_display(), o.get_payment_method_display(), o.subtotal, o.discount,
                        o.tax, o.shipping_charge, o.total, items, o.shipping_city, o.shipping_pincode])
...
class ReturnRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Decision", {"fields": ("status", "refund_amount", "admin_note"),
    ...
    @admin.action(description="✓ Approve & refund (restock + notify customer)")
    def approve_and_refund(self, request, queryset):
        done = 0
        for rr in queryset.filter(status="pending"):
            order = rr.order
            amount = rr.refund_amount or order.total
            # Best-effort online refund via Razorpay
            if order.razorpay_payment_id:
                try:
                    ...
                    self.message_user(request, f"#{order.order_number}: Razorpay refund not processed ({e}). Marked refunded — issue manually if needed.", level="warning")
            rr.status = "approved"
            rr.refund_amount = amount
            rr.save(update_fields=["status", "refund_amount"])
            order.payment_status = "refunded"
            order.status = "returned"   # signal restocks inventory + notifies
            order.save()  # triggers OrderStatusHistory via OrderAdmin? no — save directly
            OrderStatusHistory.objects.create(order=order, status="returned", note=f"Return approved · refund Rs.{amount}", created_by=request.user)
            try:
                from apps.notifications.services import notify_return_update
                notify_return_update(rr, "approved", amount)
            except Exception:
                pass
            done += 1
        self.message_user(request, f"{done} return(s) approved, refunded & restocked.")

    @admin.action(description="✗ Reject return (notify customer)")
    def reject_return(self, request, queryset):
        done = 0
        for rr in queryset.filter(status="pending"):
            rr.status = "rejected"
            rr.save(update_fields=["status"])
            # send the order back to delivered
            rr.order.status = "delivered"
            rr.order.save(update_fields=["status"])
            try:
                from apps.notifications.services import notify_return_update
                notify_return_update(rr, "rejected", None)
            except Exception:
                pass
            done += 1
